...
export default function (cv: typeof openCV): void {
  ...
  cv.getScoreMax = (scoreMat: Mat, threshold: number, region?: Rect): Array<[number, number, number]> => {
    if (scoreMat.type !== cv.CV_32F) throw Error('this method can only be call on a CV_32F Mat')
    if (scoreMat.dims !== 2) throw Error('this method can only be call on a 2 dimmention Mat')
